...
constexpr auto is_chainable_operation = [](auto type) {
    return is_filter_type(type) || is_map_type(type) || is_take_while_type(type) || is_drop_while_type(type) ||
           type == type_v<TakeNType> || type == type_v<DropNType> || type == type_v<join_t> ||
           type == type_v<reverse_t>;
};
...
constexpr auto is_container_builder = [](auto type) {
    return type == AnyOf{type_v<to_vector_t>, type_v<to_list_t>, type_v<to_deque_t>};
};
...
auto buildContainers(C &&c, T t) {
    using value = typename std::iterator_traits<decltype(begin(FWD(c)))>::value_type;
...
constexpr decltype(auto) operator|(T1 &&a, T2 &&b) {
    ...
    if constexpr (is_iterable(t1) && is_rvalue_reference(type_from(a)) && !is_range(t1)) {
        ...
        if constexpr (is_container_builder(t2)) {
            ...
            return Range{FilterIterator<it, std::decay_t<decltype(FWD(b).f)>>{beginIt, beginIt, endIt, FWD(b).f},
                         FilterIterator<it, std::decay_t<decltype(FWD(b).f)>>{endIt}};
        }

        else if constexpr (is_map_type(t2)) {
            return Range{MapIterator<it, std::decay_t<decltype(FWD(b).f)>>{beginIt, beginIt, endIt, FWD(b).f},
                         MapIterator<it, std::decay_t<decltype(FWD(b).f)>>{endIt}};
        }

        else if constexpr (is_group_by_type(t2)) {
            return Range{GroupByIterator<it, std::decay_t<decltype(FWD(b).f)>>{beginIt, beginIt, endIt, FWD(b).f},
            ...
            return FWD(b)([&a](auto &&... xs) { return (std::forward<T1>(a) | ... | (FWD(xs))); });
        }
            ...
            return Range{JoinIterator<it>{beginIt, beginIt, endIt}, JoinIterator<it>{endIt}};
        }

        else {
            compile_time_error("If a is an iterable, you must provide a range provided type, or a "
        ...
        static_assert(is_map_type(t2), "If a is an optional, you must provide map type");
        if (a)
        ...
        static_assert(is_chainable_operation(t2) || is_tuple_t(t2), "If a is a range provided type, you must provide "
        ...
        static_assert(is_map_type(t2), "If a is an optional_type, you must provide map type");
...
constexpr decltype(auto) operator>>(T1 &&a, T2 &&b) {
    ...
    if constexpr (is_iterable(t1)) {
        static_assert(is_map_type(t2), "If a is an iterable, b must be a map type joinable");
        ...
        static_assert(is_map_type(t2), "If a is an optional_type, you must provide map type");
        if_constexpr(a.has_value) { //
            typed_static_assert_msg(is_optional_type(ltl::invoke(FWD(b).f, *a)),
