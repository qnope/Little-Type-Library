...
#define TRAIT_REFERENCE(name)                                                            \
  ...
  constexpr auto LPL_CAT(name, _lifted) = [](auto &&x) constexpr { return name(FWD(x)); };
...
constexpr auto copy_cv_reference(type_t<T> type) {
  if_constexpr(is_lvalue_reference(type)) {
    if_constexpr(is_const(remove_reference(type))) { return type_v<const ResultType &>; }
    else {
...
template <typename T, typename = void> struct is_iterable_struct : false_t {};
...
template <typename T>
constexpr bool_t<is_iterable_struct<T>::value> is_iterable(type_t<T>) {
  return {};
}

template <typename T> constexpr bool_t<is_iterable_struct<T>::value> is_iterable(T &&) {
  return {};
}

constexpr auto is_iterable_lifted = [](auto &&x) { return is_iterable(FWD(x)); };
