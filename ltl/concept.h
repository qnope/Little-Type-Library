...
template <typename T> constexpr auto IsConst = decltype(is_const(std::declval<T>())){};

template <typename T>
constexpr auto IsFloatingPoint = decltype(is_floating_point(std::declval<T>())){};
...
constexpr auto IsSame =
    decltype((true_v && ... && is_same(std::declval<T>(), std::declval<Ts>()))){};
