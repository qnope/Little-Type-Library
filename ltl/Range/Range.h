...
namespace details {
template <typename R> auto callBegin(R &&r) { return begin(FWD(r)); }
template <typename R> auto callEnd(R &&r) { return end(FWD(r)); }
} // namespace details

template <typename Derived> class AbstractRange {
  ENABLE_CRTP(Derived)
public:
  bool empty() const noexcept {
    return underlying().begin() == underlying().end();
  }

  std::size_t size() const noexcept {
    return std::distance(underlying().begin(), underlying().end());
  }
...
template <typename It> class Range : public AbstractRange<Range<It>> {
public:
  template <typename R>
  Range(R &r) noexcept
      : m_it{details::callBegin(r)}, m_end{details::callEnd(r)} {}
...
class OwningRange
    : public AbstractRange<OwningRange<Container, Operations...>> {
  using range_type = decltype(std::declval<Container &>() |
                              std::declval<ltl::tuple_t<Operations...>>());
...
public:
  ...
  auto add_operation(NewOperation newOperation) && {
    return std::move(m_operations)(
        [this, &newOperation](auto &&... ops) mutable {
          return OwningRange<Container, Operations..., NewOperation>{
              std::move(m_container), FWD(ops)...,
...
template <typename R> auto begin(const AbstractRange<R> &r) noexcept {
  return static_cast<const R &>(r).begin();
}

template <typename R> auto end(const AbstractRange<R> &r) noexcept {
  return static_cast<const R &>(r).end();
}

template <typename R> auto size(const AbstractRange<R> &r) noexcept {
