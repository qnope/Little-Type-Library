...
#define ALGO_DOUBLE_ITERATOR(name)                                                       \
  ...
  auto name(C1 &&c1, C2 &&c2, As &&... as) {                                             \
    ...
    return std::name(begin(FWD(c1)), end(FWD(c1)), begin(FWD(c2)), end(FWD(c2)),         \
                     FWD(as)...);                                                        \
  }
...
#define ALGO_FIND_VALUE(name)                                                            \
  ...
  template <typename C, typename... As> auto LPL_CAT(name, _ptr)(C && c, As && ... as) { \
    auto opt = name(FWD(c), FWD(as)...);                                                 \
    if (!opt)                                                                            \
...
#define ALGO_FIND_RANGE(name)                                                            \
  ...
  auto name(C1 &&c1, C2 &&c2, As &&... as) {                                             \
    typed_static_assert_msg(is_iterable(FWD(c1)) && is_iterable(FWD(c2)),                \
                            "C1 and C2 must be iterable");                               \
    auto it = std::name(begin(FWD(c1)), end(FWD(c1)), begin(FWD(c2)), end(FWD(c2)),      \
                        FWD(as)...);                                                     \
...
template <typename C, typename... P> auto min_element_value(const C &c, P &&... p) {
  if (c.size() == 0) {
...
template <typename C, typename... P> auto max_element_value(const C &c, P &&... p) {
  if (c.size() == 0) {
    return std::decay_t<decltype(*begin(c))>{};
  }
...
template <typename C, typename... P> auto minmax_element_value(const C &c, P &&... p) {
